...
class Drv8825
{
  public:
    ...
    uint32_t get_status()
    {
      uint32_t ret = sts.read<reg::status0 + offset*0x4>();
    ...
    {
      uint32_t ret = sts.read<reg::step_count0 + offset*0x4>();
      ctx.log<INFO>("DRV8825-%s: %s=0x%08x\n", offset == 0 ? "SA" : "DC", __func__, ret);
      return ret; 
    }
    template<uint32_t offset>
    ...
    void set_backlash(uint32_t period_ticks, uint32_t n_cycle, uint8_t mode)
    {
        ...
        ctl.write<reg::backlash_tick0 + offset*0x4>(cmd);
    ...
    void set_max_step(uint32_t val){
        ...
        ctl.write<reg::counter_max0 + offset*0x4>(cmd);
    ...
    void set_current_position(uint32_t position){
        ...
        ctl.write<reg::counter_load0 + offset*0x4>(cmd);
        ctx.log<INFO>("DRV8825-%s: %s: %u cmd=0x%08x\n", offset == 0 ? "SA" : "DC", 
