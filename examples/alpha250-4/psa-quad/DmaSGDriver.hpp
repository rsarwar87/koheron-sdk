...
class AdcDma
{
  public:
    ...
    , dma_0(ctx.mm.get<mem::dma_psa>())
    , dma_1(ctx.mm.get<mem::dma_wave>())
    , ram_s2mm_0(ctx.mm.get<mem::ram_s2mm_psa>())
    , ram_s2mm_1(ctx.mm.get<mem::ram_s2mm_wave>())
    , axi_hp0(ctx.mm.get<mem::axi_hp0>())
    , axi_hp2(ctx.mm.get<mem::axi_hp2>())
    , ocm_s2mm_1(ctx.mm.get<mem::ocm_s2mm_wave>())
    , ocm_s2mm_0(ctx.mm.get<mem::ocm_s2mm_psa>())
    ...
    void set_descriptor_s2mm_1(uint32_t idx, uint32_t buffer_address, uint32_t buffer_length) {
        ocm_s2mm_1.write_reg(0x40 * idx + Sg_regs::nxtdesc, mem::ocm_s2mm_wave_addr + 0x40 * ((idx+1) % n_desc));
    ...
    void set_descriptor_s2mm_0(uint32_t idx, uint32_t buffer_address, uint32_t buffer_length) {
        ocm_s2mm_0.write_reg(0x40 * idx + Sg_regs::nxtdesc, mem::ocm_s2mm_psa_addr + 0x40 * ((idx+1) % n_desc));
    ...
    void start_dma_0() {
        ...
        for (uint32_t i = 0; i < n_desc; i++) 
            set_descriptor_s2mm_0(i, mem::ram_s2mm_psa_addr + i * 4 * n_pts, 4 * n_pts);
        // Write address of the starting descriptor
        //dma_0.write<Dma_regs::mm2s_curdesc>(mem::ocm_mm2s_addr + 0x0);
        dma_0.write<Dma_regs::s2mm_curdesc>(mem::ocm_s2mm_psa_addr + 0x0);
        ...
        dma_0.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm_psa_addr + (n_desc-1) * 0x40);
    ...
    void start_dma_1() {
        ...
        for (uint32_t i = 0; i < n_desc; i++) 
            set_descriptor_s2mm_1(i, mem::ram_s2mm_wave_addr + i * 4 * n_pts, 4 * n_pts);
        // Write address of the starting descriptor
        //dma_0.write<Dma_regs::mm2s_curdesc>(mem::ocm_mm2s_addr + 0x0);
        dma_1.write<Dma_regs::s2mm_curdesc>(mem::ocm_s2mm_wave_addr + 0x0);
        // Set DMA to cyclic mode
        //dma.set_bit<Dma_regs::s2mm_dmacr, 4>();
        // Start S2MM channel
        //dma_0.set_bit<Dma_regs::mm2s_dmacr, 0>();
        ...
        //dma.write<Dma_regs::s2mm_taildesc>(0x50);
        //dma_0.write<Dma_regs::mm2s_taildesc>(mem::ocm_s2mm1_addr + (n_desc-1) * 0x40);
        dma_1.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm_wave_addr + (n_desc-1) * 0x40);
    ...
    void start_dma_mm2s() {
        for (uint32_t i = 0; i < n_desc; i++) 
            set_descriptor_mm2s(i, mem::ram_s2mm_wave_addr + i * 4 * n_pts, 4 * n_pts);
        // Write address of the starting descriptor
        dma_1.write<Dma_regs::mm2s_curdesc>(mem::ocm_s2mm_wave_addr + 0x0);
        //dma_1.write<Dma_regs::s2mm_curdesc>(mem::ocm_s2mm_wave_addr + 0x0);
        // Set DMA to cyclic mode
        //dma.set_bit<Dma_regs::s2mm_dmacr, 4>();
        // Start S2MM channel
        dma_1.set_bit<Dma_regs::mm2s_dmacr, 0>();
        //dma_1.set_bit<Dma_regs::s2mm_dmacr, 0>();
        // Write address of the tail descriptor
        //dma.write<Dma_regs::s2mm_taildesc>(0x50);
        dma_1.write<Dma_regs::mm2s_taildesc>(mem::ocm_s2mm_wave_addr + (n_desc-1) * 0x40);
        //dma_1.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm_wave_addr + (n_desc-1) * 0x40);
    ...
    void stop_dma_mm2s() {
        dma_1.clear_bit<Dma_regs::mm2s_dmacr, 0>();
        dma_0.write<Dma_regs::mm2s_taildesc>(mem::ocm_s2mm_wave_addr + (n_desc-1) * 0x40);
    }
    auto& get_adc_data() {
    ...
    void set_dac_data(const std::vector<uint32_t>& dac_data) {
        for (uint32_t i = 0; i < dac_data.size(); i++) {
            ram_s2mm_1.write_reg(4*i, dac_data[i]);
        }
    }
    auto& get_psa_data() {
  ...
  private:
    ...
    Memory<mem::dma_psa>& dma_0;
    Memory<mem::dma_wave>& dma_1;
    Memory<mem::ram_s2mm_psa>& ram_s2mm_0;
    Memory<mem::ram_s2mm_wave>& ram_s2mm_1;
    Memory<mem::axi_hp0>& axi_hp0;
    Memory<mem::axi_hp2>& axi_hp2;
    Memory<mem::ocm_s2mm_wave>& ocm_s2mm_1;
    Memory<mem::ocm_s2mm_psa>& ocm_s2mm_0;
    ...
    void log_dma() {
        ...
        ctx.log<INFO>("CURDESC = %u \n", (dma_1.read<Dma_regs::s2mm_curdesc>() - mem::ocm_s2mm_wave_addr)/0x40);
        ctx.log<INFO>("\n");
        ...
        ctx.log<INFO>("CURDESC = %u \n", (dma_0.read<Dma_regs::s2mm_curdesc>() - mem::ocm_s2mm_psa_addr)/0x40);
        ctx.log<INFO>("\n");
