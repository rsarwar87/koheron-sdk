...
class AdcDma {
 public:
  ...
  void set_descriptor_s2mm_1(uint32_t idx, uint32_t buffer_address,
                             uint32_t buffer_length) {
    ocm_s2mm_1.write_reg(0x40 * idx + Sg_regs::nxtdesc,
  ...
  void set_descriptor_s2mm_0(uint32_t idx, uint32_t buffer_address,
                             uint32_t buffer_length) {
    ocm_s2mm_0.write_reg(0x40 * idx + Sg_regs::nxtdesc,
  ...
  void start_dma_0() {
    ...
    // 0x40);
    dma_0.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm0_addr +
                                         (n_desc - 1) * 0x40);
    if (!dma_acquisition_started) 
      log_dma();
  }
  ...
  void stop_dma_0() {
    if (dma_acquisition_started) {
      ctx.log<INFO>(
          "DMA::stop_dma_0: Thread already started, ignoring "
          "command");
      return;
    }
  ...
  void start_dma_1() {
    if (dma_acquisition_started) {
      ctx.log<INFO>(
          "DMA::start_dma_1: Acq Thread already started, ignoring "
          "command");
      return;
    }
    for (uint32_t i = 0; i < n_pts * n_desc; i++) {
    ...
    // dma_0.write<Dma_regs::mm2s_taildesc>(mem::ocm_s2mm1_addr + (n_desc-1) *
    // 0x40);
    dma_1.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm1_addr +
                                         (n_desc - 1) * 0x40);
  ...
  void stop_dma_1() {
    ...
    // dma_0.write<Dma_regs::mm2s_taildesc>(mem::ocm_mm2s_addr + (n_desc-1) *
    // 0x40);
    dma_1.write<Dma_regs::s2mm_taildesc>(mem::ocm_s2mm0_addr +
                                         (n_desc - 1) * 0x40);
  }
  ...
  auto& get_adc1_data() {
    if (dma_acquisition_started) {
      ctx.log<INFO>(
          "DMA::get_adc1_data: Acq thread already started, ignoring "
          "command");
      return data;
    }
  ...
  void start_dma_acquisition(bool circular /*, bool streaming*/) {
    if (!dma_acquisition_started) {
      ...
      dma_thread = std::thread{&AdcDma::dma_acquisition_thread, this};
      dma_thread.detach();
      // streaming_thread =
 ...
 private:
  ...
  void stream_data() {
    ...
    for (size_t i = 0; i < total_pack; i++) {
      ...
      while (ocm_s2mm_0.read_bit_reg(0x40 * idx + Sg_regs::status, 31) == 0)
        std::this_thread::sleep_for(5us);

      encoded = ram_s2mm_0.read_reg_array<uint32_t, PACK_SIZE>(i * PACK_SIZE);
...
/*    ctx.log<INFO>("DMA::DMA streaming data: starting last\n");*/
    while (ocm_s2mm_0.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 31>() ==
           0)
      std::this_thread::sleep_for(5us);
    auto encoded_ = ram_s2mm_0.read_reg_array<uint32_t, total_pack_rem>(
  ...
  void log_dma() {
    ...
    ctx.log<INFO>(
        ...
        ocm_s2mm_1.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 28>());
    ctx.log<INFO>(
        "S2MM_1_STATUS DMASlvErr = %d \n",
        ocm_s2mm_1.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 29>());
    ctx.log<INFO>(
        "S2MM_1_STATUS DMADecErr = %d \n",
        ocm_s2mm_1.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 30>());
    ctx.log<INFO>(
        "S2MM_1_STATUS Cmplt = %d \n",
        ocm_s2mm_1.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 31>());
    ctx.log<INFO>("\n");
    ctx.log<INFO>(
        "S2MM_0_STATUS DMAIntErr = %d \n",
        ocm_s2mm_0.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 28>());
    ctx.log<INFO>(
        "S2MM_0_STATUS DMASlvErr = %d \n",
        ocm_s2mm_0.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 29>());
    ctx.log<INFO>(
        "S2MM_0_STATUS DMADecErr = %d \n",
        ocm_s2mm_0.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 30>());
    ctx.log<INFO>(
        "S2MM_0_STATUS Cmplt = %d \n",
        ocm_s2mm_0.read_bit<0x40 * (n_desc - 1) + Sg_regs::status, 31>());
    ctx.log<INFO>("====================================================\n");
  }
