 ...
 public:
  ...
  void start_fifo_acquisition(bool circular /*, bool streaming*/) {
    if (!fifo_acquisition_started) {
      ...
      fifo_thread =
          std::thread{&SSFifoDriver::fifo_acquisition_thread, this};
      fifo_thread.detach();
      // streaming_thread =
      //    std::thread{&SSFifoDriver::stream_data_thread, this};
